...
resultsLR_NormalData<-resultsLR(normalDatasets)
...
resultsRR_NormalData<-resultsRobReg(normalDatasets)
...
setwd("C:/Users/nicolas/Dropbox/MLRPS Dropbox Folder/Documents by Students/Nicolas Perez/Research Manuscript/Archive")  

a1<-ggplot(data=allcoeffs, aes(x=b0ols))+
  geom_histogram(aes(x=b0ols),color="tomato",fill="tomato3",alpha=0.5)+
  geom_histogram(aes(x=b0rr),color="mediumseagreen",fill="lightseagreen",alpha=0.5)+
  labs(x=expression(Beta[0]))
pdf(file="beta0ideal.pdf", width=6, height=6)
a1
dev.off()  


a2<-ggplot(data=allcoeffs, aes(x=b1ols))+
  geom_histogram(aes(x=b1ols),color="tomato",fill="tomato3",alpha=0.5)+
  geom_histogram(aes(x=b1rr),color="mediumseagreen",fill="lightseagreen",alpha=0.5)+
  labs(x=expression(Beta[1]))
pdf(file="beta1ideal.pdf", width=6, height=6)
a2
dev.off()  


a3<-ggplot(data=allcoeffs, aes(x=b2ols))+
  geom_histogram(aes(x=b2ols),color="tomato",fill="tomato3",alpha=0.5)+
  geom_histogram(aes(x=b2rr),color="mediumseagreen",fill="lightseagreen",alpha=0.5)+
  labs(x=expression(Beta[2]))
pdf(file="beta2ideal.pdf", width=6, height=6)
a3
dev.off()  


a4<-ggplot(data=allcoeffs, aes(x=b3ols))+
...
pdf(file="beta3ideal.pdf", width=6, height=6)
a4
dev.off()  
